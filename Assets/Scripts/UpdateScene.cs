...
public class UpdateScene : MonoBehaviour
{
    ...
    public async void UpdateExp()
    {
        ...
        if (!SharedReward.distalOn)
        {
            distalObj.SetActive(false);
        }
        else
        {
            distalObj.SetActive(true);
        }
        ...
        SharedReward.juiceTime = PlayerPrefs.GetFloat("Juice Time");
